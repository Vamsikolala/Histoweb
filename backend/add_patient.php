...
if (!empty($manual_patient_id)) {
    // Check if manually provided ID already exists globally
    $check_stmt = $conn->prepare("SELECT patient_id FROM patient WHERE patient_id = ?");
    $check_stmt->bind_param("s", $manual_patient_id);
    $check_stmt->execute();
    ...
    // AUTOMATED PATIENT ID GENERATION (1, 2, 3...) globally
    $query = "SELECT patient_id FROM patient WHERE patient_id REGEXP '^[0-9]+$' ORDER BY CAST(patient_id AS UNSIGNED) DESC LIMIT 1";
