import SwiftUI

struct BreastHER2: View {
    @StateObject private var navManager = AppNavigation.shared
    @StateObject private var patientSession = PatientSessionManager.shared
    @AppStorage("doctor_id") private var doctorID: Int = 0
    
    @State private var selectedIntensity = "0"
    @State private var selectedPercentage = "> 10%"
    @State private var isIntensityExpanded = false
    @State private var isPercentageExpanded = false
    
    @State private var isSaving = false
    @State private var showSaveAlert = false
    @State private var showSavedToast = false
    @State private var alertMessage = ""

    let intensityOptions = ["0", "1+", "2+", "3+"]
    let percentageOptions = ["> 10%", "≤ 10%"]

    var inference: String {
        if selectedIntensity == "3+" {
            return selectedPercentage == "> 10%" ? "Positive" : "Equivocal"
        } else if selectedIntensity == "2+" {
            return selectedPercentage == "> 10%" ? "Equivocal" : "Equivocal" // Both 2+ conditions in screenshot are Equivocal (though the first one is explicit)
        } else if selectedIntensity == "1+" {
            return selectedPercentage == "> 10%" ? "Negative (1+)" : "Negative (0)"
        } else {
            return "Negative (0)"
        }
    }
    
    var inferenceColor: Color {
        if inference.contains("Positive") { return .green }
        if inference.contains("Equivocal") { return .orange }
        if inference.contains("Negative") { return .red }
        return .gray
    }

    var body: some View {
        ZStack {
            VStack(spacing: 0) {
                StandardHeader(title: "Breast - HER2") {
                    Button(action: {
                        navManager.navigate(to: .her2Guidelines)
                    }) {
                        ZStack {
                            Circle()
                                .fill(Color.white.opacity(0.18))
                                .frame(width: 38, height: 38)
                            Image(systemName: "ellipsis")
                                .font(.system(size: 18, weight: .bold))
                                .foregroundColor(.white)
                        }
                    }
                }

                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        titleCard

                        VStack(alignment: .leading, spacing: 20) {
                            intensityDropdown
                            percentageDropdown
                        }
                        .padding(18)
                        .background(Color.white)
                        .cornerRadius(18)
                        .shadow(color: Color.black.opacity(0.05), radius: 8, x: 0, y: 3)

                        // Result Card
                        VStack(spacing: 8) {
                            Text("Interpretation: \(inference)")
                                .font(.system(size: 20, weight: .bold))
                                .foregroundColor(inferenceColor)
                            
                            Text(selectedIntensity == "2+" ? "Further FISH testing recommended" : (inference == "Positive" ? "HER2 gene amplification present" : "No HER2 gene amplification"))
                                .font(.system(size: 14))
                                .foregroundColor(.gray)
                                .multilineTextAlignment(.center)
                        }
                        .frame(maxWidth: .infinity)
                        .padding(20)
                        .background(Color.white)
                        .cornerRadius(18)
                        .overlay(
                            RoundedRectangle(cornerRadius: 18)
                                .stroke(inferenceColor.opacity(0.2), lineWidth: 2)
                        )
                        .shadow(color: Color.black.opacity(0.05), radius: 8, x: 0, y: 3)

                        if let patient = patientSession.currentPatient {
                            Button(action: saveAnalysis) {
                                HStack {
                                    if isSaving {
                                        ProgressView().tint(.white).padding(.trailing, 8)
                                    } else {
                                        Image(systemName: "checkmark.circle.fill")
                                    }
                                    Text("Save to \(patient.name)'s Report")
                                        .fontWeight(.bold)
                                }
                                .foregroundColor(.white)
                                .frame(maxWidth: .infinity, minHeight: 54)
                                .background(LinearGradient(colors: [Color.blue, Color.cyan], startPoint: .leading, endPoint: .trailing))
                                .cornerRadius(16)
                                .shadow(color: Color.blue.opacity(0.3), radius: 8)
                            }
                            .disabled(isSaving)
                            .padding(.top, 10)
                        }

                        interpretationCard
                    }
                    .padding(.horizontal, 18)
                    .padding(.top, 18)
                    .padding(.bottom, 24)
                }
            }
            .ignoresSafeArea(edges: .top)
            
            // Toast Overlay
            if showSavedToast {
                VStack {
                    Spacer()
                    HStack {
                        Image(systemName: "checkmark.circle.fill")
                        Text("Analysis Saved")
                    }
                    .font(.system(size: 16, weight: .bold))
                    .foregroundColor(.white)
                    .padding(.vertical, 12)
                    .padding(.horizontal, 24)
                    .background(Color.green)
                    .cornerRadius(25)
                    .shadow(radius: 5)
                    .padding(.bottom, 50)
                }
                .transition(.move(edge: .bottom).combined(with: .opacity))
                .zIndex(100)
            }
        }
        .background(
            LinearGradient(
                colors: [
                    Color(red: 0.95, green: 0.97, blue: 1.0),
                    Color.white
                ],
                startPoint: .top,
                endPoint: .bottom
            )
            .ignoresSafeArea()
        )
        .alert("Status", isPresented: $showSaveAlert) {
            Button("OK", role: .cancel) { }
        } message: {
            Text(alertMessage)
        }
        .navigationBarBackButtonHidden(true)
        .toolbar(.hidden, for: .navigationBar)
    }



    @ViewBuilder
    var intensityDropdown: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("Select Intensity of Staining:")
                .font(.system(size: 16, weight: .semibold))
                .foregroundColor(.black)
                .padding(.horizontal, 4)
                .padding(.bottom, 12)
            
            VStack(spacing: 0) {
                Button(action: { 
                    withAnimation(.easeInOut(duration: 0.2)) {
                        isIntensityExpanded.toggle()
                        if isIntensityExpanded { isPercentageExpanded = false }
                    }
                }) {
                    HStack {
                        Text(selectedIntensity)
                            .font(.system(size: 17))
                            .foregroundColor(.black)
                        Spacer()
                        Image(systemName: "chevron.down")
                            .font(.system(size: 14, weight: .bold))
                            .foregroundColor(.blue)
                            .rotationEffect(.degrees(isIntensityExpanded ? 180 : 0))
                    }
                    .padding(.vertical, 14)
                    .padding(.horizontal, 16)
                    .background(Color.white)
                }
                .clipShape(RoundedCorner(radius: 16, corners: isIntensityExpanded ? [.topLeft, .topRight] : .allCorners))
                .shadow(color: Color.black.opacity(0.06), radius: 6, x: 0, y: 3)
                .overlay(
                    RoundedCorner(radius: 16, corners: isIntensityExpanded ? [.topLeft, .topRight] : .allCorners)
                        .stroke(Color.gray.opacity(0.15), lineWidth: 1.2)
                )
                
                if isIntensityExpanded {
                    VStack(spacing: 0) {
                        ForEach(intensityOptions, id: \.self) { option in
                            Button(action: {
                                selectedIntensity = option
                                withAnimation { isIntensityExpanded = false }
                            }) {
                                HStack {
                                    Text(option)
                                        .font(.system(size: 16))
                                        .foregroundColor(selectedIntensity == option ? .blue : .black.opacity(0.8))
                                    Spacer()
                                    if selectedIntensity == option {
                                        Image(systemName: "checkmark")
                                            .font(.system(size: 14, weight: .bold))
                                            .foregroundColor(.blue)
                                    }
                                }
                                .padding(.vertical, 14)
                                .padding(.horizontal, 16)
                                .background(selectedIntensity == option ? Color.blue.opacity(0.05) : Color.white)
                            }
                            if option != intensityOptions.last {
                                Divider().padding(.horizontal, 16)
                            }
                        }
                    }
                    .background(Color.white)
                    .clipShape(RoundedCorner(radius: 16, corners: [.bottomLeft, .bottomRight]))
                    .shadow(color: Color.black.opacity(0.06), radius: 10, x: 0, y: 8)
                    .transition(.opacity)
                    .overlay(
                        RoundedCorner(radius: 16, corners: [.bottomLeft, .bottomRight])
                            .stroke(Color.gray.opacity(0.15), lineWidth: 1.2)
                    )
                }
            }
        }
        .zIndex(isIntensityExpanded ? 20 : 1)
    }

    @ViewBuilder
    var percentageDropdown: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("Percentage of Tumor Cells:")
                .font(.system(size: 16, weight: .semibold))
                .foregroundColor(.black)
                .padding(.horizontal, 4)
                .padding(.bottom, 12)
            
            VStack(spacing: 0) {
                Button(action: { 
                    withAnimation(.easeInOut(duration: 0.2)) {
                        isPercentageExpanded.toggle()
                        if isPercentageExpanded { isIntensityExpanded = false }
                    }
                }) {
                    HStack {
                        Text(selectedPercentage)
                            .font(.system(size: 17))
                            .foregroundColor(.black)
                        Spacer()
                        Image(systemName: "chevron.down")
                            .font(.system(size: 14, weight: .bold))
                            .foregroundColor(.blue)
                            .rotationEffect(.degrees(isPercentageExpanded ? 180 : 0))
                    }
                    .padding(.vertical, 14)
                    .padding(.horizontal, 16)
                    .background(Color.white)
                }
                .clipShape(RoundedCorner(radius: 16, corners: isPercentageExpanded ? [.topLeft, .topRight] : .allCorners))
                .shadow(color: Color.black.opacity(0.06), radius: 6, x: 0, y: 3)
                .overlay(
                    RoundedCorner(radius: 16, corners: isPercentageExpanded ? [.topLeft, .topRight] : .allCorners)
                        .stroke(Color.gray.opacity(0.15), lineWidth: 1.2)
                )
                
                if isPercentageExpanded {
                    VStack(spacing: 0) {
                        ForEach(percentageOptions, id: \.self) { option in
                            Button(action: {
                                selectedPercentage = option
                                withAnimation { isPercentageExpanded = false }
                            }) {
                                HStack {
                                    Text(option)
                                        .font(.system(size: 16))
                                        .foregroundColor(selectedPercentage == option ? .blue : .black.opacity(0.8))
                                    Spacer()
                                    if selectedPercentage == option {
                                        Image(systemName: "checkmark")
                                            .font(.system(size: 14, weight: .bold))
                                            .foregroundColor(.blue)
                                    }
                                }
                                .padding(.vertical, 14)
                                .padding(.horizontal, 16)
                                .background(selectedPercentage == option ? Color.blue.opacity(0.05) : Color.white)
                            }
                            if option != percentageOptions.last {
                                Divider().padding(.horizontal, 16)
                            }
                        }
                    }
                    .background(Color.white)
                    .clipShape(RoundedCorner(radius: 16, corners: [.bottomLeft, .bottomRight]))
                    .shadow(color: Color.black.opacity(0.06), radius: 10, x: 0, y: 8)
                    .transition(.opacity)
                    .overlay(
                        RoundedCorner(radius: 16, corners: [.bottomLeft, .bottomRight])
                            .stroke(Color.gray.opacity(0.15), lineWidth: 1.2)
                    )
                }
            }
        }
        .zIndex(isPercentageExpanded ? 19 : 1)
    }

    var titleCard: some View {
        HStack {
            Image(systemName: "cross.case.fill")
                .foregroundColor(.blue)
                .font(.system(size: 22))

            Text("HER2")
                .font(.system(size: 20, weight: .bold))
                .foregroundColor(.black)

            Spacer()
        }
        .padding(16)
        .background(Color.white)
        .cornerRadius(18)
        .shadow(color: Color.black.opacity(0.05), radius: 8, x: 0, y: 3)
    }



    var interpretationCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Quick Interpretation")
                .font(.system(size: 18, weight: .bold))
                .foregroundColor(.black)

            interpretationRow(text: "3+ → Positive", color: .green)
            interpretationRow(text: "2+ → Equivocal", color: .orange)
            interpretationRow(text: "1+ / 0 → Negative", color: .red)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(18)
        .background(Color.white)
        .cornerRadius(18)
        .shadow(color: Color.black.opacity(0.05), radius: 8, x: 0, y: 3)
    }



    @ViewBuilder
    func interpretationRow(text: String, color: Color) -> some View {
        HStack(spacing: 10) {
            Circle()
                .fill(color)
                .frame(width: 10, height: 10)

            Text(text)
                .font(.system(size: 15, weight: .medium))
                .foregroundColor(.black)
        }
    }
    
    func saveAnalysis() {
        guard doctorID > 0 else { alertMessage = "Login Error: Doctor ID not found. Please log out and log in again."; showSaveAlert = true; return }
        guard let patient = patientSession.currentPatient else { alertMessage = "No Patient Selected: Please search for and select a patient before saving."; showSaveAlert = true; return }
        isSaving = true
        
        let reportType = "Breast - HER2 Analysis"
        let diagnosisText = "HER2 \(inference)"
        let notesText = "Intensity: \(selectedIntensity), Cells: \(selectedPercentage)"
        let fullReport = """
        CLINICAL ANALYSIS REPORT: BREAST HER2
        -------------------------------------------
        Patient Name: \(patient.name)
        Patient ID: \(patient.patient_id)
        
        STAINING INTENSITY: \(selectedIntensity)
        PERCENTAGE OF TUMOR CELLS: \(selectedPercentage)
        
        INFERENCE: \(inference)
        
        Interpretation: \(selectedIntensity == "2+" ? "The result is Equivocal. Reflex testing using FISH is strongly recommended for final classification." : (inference == "Positive" ? "HER2 overexpression is detected. The patient may be a candidate for anti-HER2 targeted therapy." : "HER2 negative. No clinical indication for targeted therapy."))
        """
        
        
        let url = NetworkManager.shared.getURL(for: "add_analysis_report.php")
        
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: "Content-Type")
        
        let bodyParameters = [
            "doctor_id": "\(doctorID)",
            "patient_id": patient.patient_id,
            "tissue_type": "Breast",
            "marker": "HER2",
            "total_score": "Score \(selectedIntensity)",
            "inference": fullReport
        ]
        
        let bodyString = bodyParameters.map { "\($0.key)=\($0.value.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")" }.joined(separator: "&")
        request.httpBody = bodyString.data(using: .utf8)
        
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                isSaving = false
                if let error = error {
                    alertMessage = "Network error: \(error.localizedDescription)"
                    showSaveAlert = true
                } else if let data = data, let result = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    if result["status"] as? Bool == true {
                        withAnimation {
                            showSavedToast = true
                        }
                        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
                            withAnimation {
                                showSavedToast = false
                            }
                        }
                    } else {
                        alertMessage = "Error: \(result["message"] ?? "Unknown error")"
                        showSaveAlert = true
                    }
                } else {
                    alertMessage = "Invalid response from server"
                    showSaveAlert = true
                }
            }
        }.resume()
    }
}

struct BreastHER2_Previews: PreviewProvider {
    static var previews: some View {
        NavigationStack {
            BreastHER2()
        }
    }
}
