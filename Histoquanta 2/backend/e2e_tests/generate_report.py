"""
generate_report.py - Generates a professional E2E Test Report in .xlsx format
=============================================================================
Mirrors the reference format from E2E_Test_Report_PancreaScan.
Generates a timestamped Excel file with:
  - Summary Sheet: Overview stats, pass/fail pie chart area
  - Detailed Results Sheet: All 130 test cases with full details
  - Module-wise Sheet: Aggregated pass/fail per module
"""
...
def generate_excel_report(results, output_dir=None):
    """Generate the .xlsx E2E test report from collected results."""

    if output_dir is None:
        output_dir = os.path.dirname(os.path.abspath(__file__))

    timestamp = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    filename = f"E2E_Test_Report_HistoQuanta_{timestamp}.xlsx"
    ...
    title_font = Font(name="Calibri", bold=True, size=18, color="0F4C81")
    ...
    ws1 = wb.active
    ws1.title = "Summary"
    ws1.sheet_properties.tabColor = "0F4C81"

    # Title
    ws1.merge_cells("A1:H1")
    ws1["A1"] = "HistoQuanta - E2E Test Report"
    ...
    summary_data = [
        ("Metric", "Value"),
        ("Application", "HistoQuanta Web (Cancer Screening Portal)"),
        ("Test Date", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        ("Browser", "Google Chrome (Selenium Automated)"),
...
if __name__ == "__main__":
    ...
    generate_excel_report(sample_results)
