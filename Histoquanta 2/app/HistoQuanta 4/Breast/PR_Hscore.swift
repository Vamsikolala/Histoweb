...
struct PR_HScore: View {
    ...
    var formulaCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack { Text("1 × [% Weak]"); Spacer() ; Text("\(1 * weakVal)") }.foregroundColor(.gray)
                HStack { Text("2 × [% Moderate]"); Spacer() ; Text("\(2 * moderateVal)") }.foregroundColor(.gray)
                HStack { Text("3 × [% Strong]"); Spacer() ; Text("\(3 * strongVal)") }.foregroundColor(.gray)
            }
            .font(.system(size: 15))
        }
    ...
    func saveReport(for patient: HQPatient) {
        ...
        Interpretation: \(totalHScore >= 10 ? "The sample shows PR reactivity. Higher H-scores (especially >100) are strongly associated with hormone therapy response." : "The sample is considered negative by PR H-score criteria.")
        """
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    if result["status"] as? Bool == true {
                        alertMessage = "PR H-Score report saved successfully to patient history!"
                    } else {
...
struct BreastPR_HScore_Previews: PreviewProvider {
    static var previews: some View {
        NavigationStack {
            PR_HScore(initialIntensity: 2, initialProportion: 3)
        }
    }
}
