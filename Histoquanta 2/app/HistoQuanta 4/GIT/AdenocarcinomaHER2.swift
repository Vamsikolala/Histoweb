...
struct AdenocarcinomaHER2: View {
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 0) {
                StandardHeader(title: "GIT Analysis") {
                    Button(action: { navManager.navigate(to: .gitGuidelines) }) {
                        ZStack {
                            Circle()
    ...
    func saveAnalysis() {
        ...
        let reportType = "GIT - Adenocarcinoma HER2"
        let diagnosisText = "HER2 \(scoreCategory)"
        let notesText = "Intensity: \(selectedIntensity.prefix(2))"
        let fullReport = """
        CLINICAL ANALYSIS REPORT: GIT ADENOCARCINOMA HER2
        -------------------------------------------
        ...
        let bodyParameters = [
            ...
            "tissue_type": "GIT",
            "marker": "Adenocarcinoma HER2 (Surgical)",
...
struct AdenocarcinomaHER2_Previews: PreviewProvider {
