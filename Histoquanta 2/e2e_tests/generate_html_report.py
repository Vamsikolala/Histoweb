...
def generate_html_report(results, output_dir=None):
    """Generate a premium interactive HTML report of the test results."""
    if output_dir is None:
        output_dir = os.path.dirname(os.path.abspath(__file__))

    timestamp = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    filename = f"E2E_Test_Report_HistoQuanta_{timestamp}.html"
...
<head>
    ...
    <title>HistoQuanta - E2E Test Report</title>
    ...
    <div class="container">
        <header>
            <div class="title-area">
                <h1>HistoQuanta Automated Test Report</h1>
                <p>Comprehensive End-to-End Test Suite Status Dashboard</p>
            </div>
            <div class="timestamp">
    ...
    for tc in sorted(results, key=lambda x: int(x["test_id"].split("_")[1]) if "_" in x.get("test_id", "") else 999):
        ...
        html_content += f"""
                    <tr class="result-row" data-status="{tc.get('status')}">
                        ...
                        <td><span style="font-weight:600; color: #475569;">{tc.get('module', '')}</span></td>
                        <td>
                        ...
                        <td>
                            <div class="tc-steps"><span style="font-weight:600;color:#334155;">Steps:</span>\n{tc.get('steps', '')}</div>
                            <div style="margin-top:6px; font-size:0.875rem;"><span style="font-weight:600;color:#334155;">Expected:</span> {tc.get('expected_result', '')}</div>
                        </td>
...
"""
    ...
    print(f"\n============================================================")
    print(f"  E2E HTML TEST REPORT GENERATED SUCCESSFULLY!")
    print(f"============================================================")
