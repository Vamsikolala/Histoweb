...
export const PRForm: React.FC = () => {
  ...
  const handleSaveAndHScore = async () => {
    ...
    try {
      ...
      const fullReport = `CLINICAL ANALYSIS REPORT: BREAST PR (ALLRED SCORE)
-------------------------------------------
...
Interpretation: ${totalScore <= 2 ? 'The sample is PR Negative. Hormone therapy is unlikely to be beneficial.' : 'The sample is PR Positive. The patient may benefit from hormone-based therapy.'}`;
  ...
  return (
    <Layout 
      title="Breast Pathology - Progesterone Receptor (PR)"
      actions={
        <button 
        ...
        <div style={{ 
          ...
          <div>
            ...
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', margin: '2px 0 0', fontWeight: 500 }}>
              Standardized semi-quantitative scoring for Progesterone Receptor (PR) expression
            </p>
          </div>
        </div>
        ...
        <div className="card animate-fade-in-up" style={{ padding: 'var(--sp-6)', borderRadius: '24px', boxShadow: 'var(--shadow)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', margin: '4px 0 0', maxWidth: '440px', lineHeight: 1.4, fontWeight: 500 }}>
              {totalScore <= 2 
                ? 'The specimen demonstrates negative Progesterone Receptor expression (Allred Score ≤ 2). Endocrine/Hormonal therapy is unlikely to yield clinical benefit.' 
                : 'The specimen demonstrates positive Progesterone Receptor expression (Allred Score 3–8). Endocrine/Hormonal therapy (e.g. Tamoxifen/Aromatase inhibitors) is highly recommended.'}
            </p>
          </div>
