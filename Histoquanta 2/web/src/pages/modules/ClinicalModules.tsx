...
const clinicalData = [
  {
    tissue: 'Breast',
    markers: ['ER (Allred)', 'ER (H-Score)', 'PR (Allred)', 'PR (H-Score)', 'HER2', 'Ki67']
  },
  {
    tissue: 'GIT',
    markers: ['Adenocarcinoma', 'NET', 'GIST']
  },
  {
    tissue: 'Lungs',
    markers: ['HER2']
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {clinicalData.find(d => d.tissue === selectedTissue)?.markers.map((marker) => (
                <div 
                  ...
                  className="glass-card"
                  onClick={() => {
                    const base = `/patient/${id}/modules`;
                    // Breast markers
                    if (selectedTissue === 'Breast' && marker === 'ER (Allred)') {
                      navigate(`${base}/breast/er`);
                    } else if (selectedTissue === 'Breast' && marker === 'ER (H-Score)') {
                      navigate(`${base}/breast/er-hscore`);
                    } else if (selectedTissue === 'Breast' && marker === 'PR (Allred)') {
                      navigate(`${base}/breast/pr`);
                    } else if (selectedTissue === 'Breast' && marker === 'PR (H-Score)') {
                      navigate(`${base}/breast/pr-hscore`);
                    } else if (selectedTissue === 'Breast' && marker === 'HER2') {
                      navigate(`${base}/breast/her2`);
                    } else if (selectedTissue === 'Breast' && marker === 'Ki67') {
                      navigate(`${base}/breast/ki67`);
                    }
                    // GIT markers
                    else if (selectedTissue === 'GIT' && marker === 'Adenocarcinoma') {
                      navigate(`${base}/git/adenocarcinoma`);
                    } else if (selectedTissue === 'GIT' && marker === 'NET') {
                      navigate(`${base}/git/net`);
                    } else if (selectedTissue === 'GIT' && marker === 'GIST') {
                      navigate(`${base}/git/gist`);
                    }
                    // Other tissues (single marker each, go directly)
                    else if (selectedTissue === 'Lungs') {
                      navigate(`${base}/lungs`);
                    } else if (selectedTissue === 'Thyroid') {
                      navigate(`${base}/thyroid`);
                    } else if (selectedTissue === 'Head & Neck' && marker === 'P16') {
                      navigate(`${base}/headneck/p16`);
                    } else if (selectedTissue === 'Head & Neck' && marker === 'HER2') {
                      navigate(`${base}/headneck/her2`);
                    } else if (selectedTissue === 'Soft Tissue') {
                      navigate(`${base}/softtissue`);
                    } else {
