...
export const HeadneckHER2: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const [selectedResult, setSelectedResult] = useState<string>("0 negative");
  const [isExpanded, setIsExpanded] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  let inference = "Negative";
  if (selectedResult === "3+ positive") inference = "Positive";
  else if (selectedResult === "2+ equivocal") inference = "Equivocal";

  let resultText = "No staining observed.\nHER2 negative — standard chemotherapy preferred.";
  if (selectedResult === "3+ positive") resultText = "Strong complete membrane staining in >10% of tumor cells.\nHER2 amplified — consider targeted therapy (e.g., trastuzumab + chemo).";
  else if (selectedResult === "2+ equivocal") resultText = "Moderate incomplete membrane staining.\nRequires FISH/CISH confirmation for HER2 amplification status.";
  else if (selectedResult === "1+ negative") resultText = "Weak incomplete membrane staining.\nHER2 negative — no targeted therapy indicated.";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const fullReport = `CLINICAL ANALYSIS REPORT: HEAD & NECK HER2
-------------------------------------------
    ...
    <Layout title="Head & Neck">
      ...
      <div style={{ 
        ...
        <div style={{ margin: '0 18px 24px', padding: '20px', backgroundColor: '#fff', borderRadius: '18px', boxShadow: '0 3px 8px rgba(0,0,0,0.05)' }}>
          <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#000', marginBottom: '15px' }}>INTERPRETATION CRITERIA:</div>
